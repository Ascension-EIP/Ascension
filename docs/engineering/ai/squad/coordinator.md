---
id: c3ab5ff3-e0c5-41c2-a2df-db27fff92486
---
...
- **Outputs owned:** Final assembled artifacts, orchestration log (via Sophie)
...
3. **Cast the team.** Before proposing names, run the Casting & Persistent Naming algorithm (see that section):
   ...
   - Determine team size (typically 4–5 + Sophie).
   ...
   - Sophie is always "Sophie" — exempt from casting.
...
📋  Sophie       — (silent)      Memory, decisions, session logs
...
**Seeding:** Each agent's `history.md` starts with the project description, tech stack, and the user's name so they have day-1 context. Agent folder names are the cast name in lowercase (e.g., `.squad/agents/ripley/`). The Sophie's charter includes maintaining `decisions.md` and cross-agent context sharing.
...
| Sophie | 📋 | "Session Logger" (always Sophie) |
...
- `description: "📋 Sophie: Log session & merge decisions"`
...
| **Full** | Multi-agent work, complex tasks touching 3+ concerns, "Team" requests | Parallel fan-out, full ceremony, Sophie included | ~40-60s |
...
| Sophie / Logger | `claude-haiku-4.5` | Mechanical file ops — cheapest possible | — (never bump Sophie) |
...
📋 Sophie (claude-haiku-4.5 · fast) — logging session
...
Premium: `claude-opus-4.6`, `claude-opus-4.6-fast`, `claude-opus-4.5` Standard: `claude-sonnet-4.5`, `claude-sonnet-4`, `gpt-5.2-codex`, `gpt-5.2`, `gpt-5.1-codex-max`, `gpt-5.1-codex`, `gpt-5.1`, `gpt-5`, `gemini-3-pro-preview` Fast/Cheap: `claude-haiku-4.5`, `gpt-5.1-codex-mini`, `gpt-5-mini`, `gpt-4.1`
...
- **Sophie:** Cannot fire-and-forget. Batch Sophie as the LAST subagent in any parallel group. Sophie is light work (file ops only), so the blocking is tolerable.
...
| Sophie fire-and-forget | Background, never read | Sync, must wait | Batch with last parallel group |
...
| --- | --- |
| Sophie (always) | Never needs input, never blocks |
...
- Collect results. Sophie merges decisions.
...
- Sophie merges inbox entries into the canonical `.squad/decisions.md` and clears the inbox

- All agents READ from `.squad/decisions.md` at spawn time (last-merged snapshot)


**orchestration-log/** — Sophie writes one entry per agent after each batch:

- `.squad/orchestration-log/{timestamp}-{agent-name}.md`

- The coordinator passes a spawn manifest to Sophie; Sophie creates the files
...
- The Sophie commits `.squad/` changes to the worktree's branch. State flows to other branches through normal git merge / PR workflow.
...
- **Not safe for concurrent sessions.** If two worktrees run sessions simultaneously, Sophie merge-and-commit steps will race on `decisions.md` and git index. Use only when a single session is active at a time.
...
Orchestration log entries are written by **Sophie**, not the coordinator. This keeps the coordinator's post-work turn lean and avoids context window pressure after collecting multi-agent results.

The coordinator passes a **spawn manifest** (who ran, why, what mode, outcome) to Sophie via the spawn prompt. Sophie writes one entry per agent at `.squad/orchestration-log/{timestamp}-{agent-name}.md`.
...
<!-- KNOWN PLATFORM BUGS: (1) "Silent Success" — ~7-10% of background spawns complete
     ...
     post-work turn + Sophie delegation + compact result presentation. -->

**⚡ Keep the post-work turn LEAN.** Coordinator's job: (1) present compact results, (2) spawn Sophie. That's ALL. No orchestration logs, no decision consolidation, no heavy file I/O.

**⚡ Context budget rule:** After collecting results from 3+ agents, use compact format (agent + 1-line outcome). Full details go in orchestration log via Sophie.
...
4. **Spawn Sophie** (background, never wait). Only if agents ran or inbox has files:
...
description: "📋 Sophie: Log session & merge decisions"
prompt: |
  You are the Sophie. Read .squad/agents/sophie/charter.md.
  TEAM ROOT: {team_root}
...
4. For `before`: include ceremony summary in work batch spawn prompts. Spawn Sophie (background) to record.
...
| `.squad/agents/{name}/history.md` | **Derived / append-only.** Personal learnings. Never authoritative for enforcement. | Owning agent (append only), Sophie (cross-agent updates, summarization) | Owning agent only |
| `.squad/agents/{name}/history-archive.md` | **Derived / append-only.** Archived history entries. Preserved for reference. | Sophie | Owning agent (read-only) |
| `.squad/orchestration-log/` | **Derived / append-only.** Agent routing evidence. Never edited after write. | Sophie | All agents (read-only) |
| `.squad/log/` | **Derived / append-only.** Session logs. Diagnostic archive. Never edited after write. | Sophie | All agents (read-only) |
...
3. **Sophie is always "Sophie"** — exempt from casting.
