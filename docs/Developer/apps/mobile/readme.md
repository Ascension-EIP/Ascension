---
id: 5da83c2d-e8a1-41b0-a989-d2652ca1923d
---
...
All HTTP calls to the Go backend go through `ApiService` (`lib/core/network/api_service.dart`). It is a Dart singleton — call `ApiService()` from anywhere to get the same instance.
...
The video is **never proxied through the Go server**. It goes directly from the device to MinIO using the presigned URL. The Go server only creates the database record and returns the URL.
